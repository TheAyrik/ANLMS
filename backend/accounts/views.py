...
class CookieTokenRefreshView(TokenRefreshView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if refresh is None:
            return Response({'detail': 'توکن رفرش در کوکی یافت نشد.'}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        response = Response({'detail': 'توکن با موفقیت به روزرسانی شد.'}, status=status.HTTP_200_OK)
...
class LogoutView(APIView):
    ...
    def post(self, request):
        response = Response({'detail': 'خروج با موفقیت انجام شد.'}, status=status.HTTP_200_OK)
